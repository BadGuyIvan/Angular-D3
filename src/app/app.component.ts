...
export class AppComponent implements OnInit {
  ...
  randomData(samples) {
    this.lineChart = [];
    let data = [];
    let count = this.getRandomArbitrary(1, 5);
    for (let index = 0; index < count; index++) {
      for (let i = 0; i < samples; i++) {
        if(this.lineChart.length){
          data.push({
            x: i,
            y: this.getRandomArbitrary(Math.min(...this.lineChart[0].data.map(d => d.y)),Math.max(...this.lineChart[0].data.map(d => d.y)))
          });
        } else {
          data.push({
            x: i,
      ...
      this.lineChart.push({ data, color: this.color[index] });
      data = [];
    }
  ...
  changedDataPieChart() {
    this.randomData(this.getRandomArbitrary(1,50));
