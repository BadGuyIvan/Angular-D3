...
export class ArcChartComponent implements OnInit, OnChanges {
  ...
  update(newValue, prevValue){

    //same size array in data

    // this.chart
    // .select('.arcs')
    // .selectAll('path')
    // .data(this.pie(newValue))
    // .attr("d", this.arc)
    // .transition()
    //   .duration(1000)
    //   .attrTween("d", (d,index) => {
    //     let oldValue = this.pie(prevValue);
    //     var interpolate = d3.interpolate(oldValue[index], d);
    //     return (t) => {
    //       return this.arc(interpolate(t));
    //     };
    //   })
    ...
    // delete slice which it be deprecated
    update.exit()
      .transition()
      .duration(1000)
      .attrTween('d', (d) => {
    ...
    .enter()
      ...
      .duration(1000)
      .attrTween("d", (d) => {
        var start = { startAngle: 2 * Math.PI, endAngle: 2 * Math.PI };
        var interpolate = d3.interpolate(start, d);
        return (t) => {
          return this.arc(interpolate(t));
        };
  ...
  ngOnChanges(changes: SimpleChanges) {
    if(changes.data.previousValue !== undefined){
      this.update(changes.data.currentValue, changes.data.previousValue);
    }
  }
